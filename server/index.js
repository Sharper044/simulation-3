...
passport.use(new Auth0Strategy({
  ...
  db.find_user([ profile.sub ])
    .then( user => {
      if ( user[0] ) {
        return done( null, { id: user[0].user_id });
      }
      else {
        db.create_user([ profile.sub , profile.given_name, lastname.family_name ])
          .then( user => {
...
app.listen(process.env.SERVER_PORT, () => console.log(`Hailing frequencies open on port ${SERVER_PORT}...`));